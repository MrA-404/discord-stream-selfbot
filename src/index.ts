            ...
            case 'play':
{
    ...
    const isMute = args.includes('mute');  // ✅ detect mute flag
    ...
    logger.info(`Starting looped play of local video: ${video.path}`);

    const loopCount = 5;

    for (let i = 0; i < loopCount; i++) {
        logger.info(`▶️ Loop ${i + 1} of ${loopCount}`);
        await playVideo(message, video.path, `${videoname} (loop ${i + 1})`, isMute);  // ✅ pass mute flag
    }

    logger.info(`✅ Finished playing ${videoname} in a loop ${loopCount} times.`);
}
break;
...
async function playVideo(message: Message, videoSource: string, title?: string, mute: boolean = false) {
